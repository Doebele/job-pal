...
router.get('/', async (c) => {
  ...
  let whereClause: any;
  if (role === 'employer') {
    whereClause = query.jobId
      ? and(eq(applications.jobId, query.jobId), eq(jobs.employerId, userId))
...
router.put('/:id/status', async (c) => {
  ...
  // Nur der Arbeitgeber des zugehoerigen Jobs darf den Bewerbungsstatus aendern.
